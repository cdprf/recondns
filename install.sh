...
declare -A gotools
gotools["uro"]="go install -v github.com/evilsocket/uroboros@lastet"
...
if [[ $(id -u | grep -o '^0$') == "0" ]]; then
    ...
    if sudo -n false 2>/dev/null; then
        ...
        printf "${bred} echo \"${USERNAME}  ALL=(ALL:ALL) NOPASSWD: ALL\" > /etc/sudoers.d/recondns${reset}\n\n"
    fi
    SUDO="sudo"
fi
...
printf "${bblue} Running: Looking for new recondns version${reset}\n\n"
...
then
    ...
    if [ -n "$(git status --porcelain | egrep 'recondns.cfg$')" ]; then
        mv recondns.cfg recondns.cfg_bck
        printf "${yellow} recondns.cfg has been backed up in recondns.cfg_bck${reset}\n\n"
    fi
    eval git reset --hard $DEBUG_STD
    eval git pull $DEBUG_STD
    printf "${bgreen} Updated! Running new installer version...${reset}\n\n"
else
    printf "${bgreen} recondns is already up to date!${reset}\n\n"
fi
