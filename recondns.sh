...
function banner(){
	...
	printf "    ░        ░  ░░ ░          ░ ░           ░                      ░    \n"
	printf "                 ░                                                      \n"
	printf " ${recondns_version}                                 by @omar Passos${reset}\n"
}
...
function google_dorks(){
	if [ ! -f "$called_fn_dir/.${FUNCNAME[0]}" ] && [ "$GOOGLE_DORKS" = true ] && [ "$OSINT" = true ]; then
		...
		if [ "$GOOGLE_DORKS" = false ] || [ "$OSINT" = false ]; then
			printf "\n${yellow} ${FUNCNAME[0]} skipped in this mode or defined in recondns.cfg ${reset}\n"
		else
...
function sub_recursive(){
	if { [ ! -f "$called_fn_dir/.${FUNCNAME[0]}" ] || [ "$DIFF" = true ]; } && [ "$SUBRECURSIVE" = true ] && [ -s "subdomains/subdomains.txt" ]; then
		...
		if [[ $(cat subdomains/subdomains.txt | wc -l) -le $DEEP_LIMIT ]] && [ "$SUB_RECURSIVE_BRUTE" = true ] ; then
			...
			end_subfunc "skipped in this mode or defined in recondns.cfg" ${FUNCNAME[0]}
		fi
	...
	if { [ ! -f "$called_fn_dir/.${FUNCNAME[0]}" ] || [ "$DIFF" = true ]; } && [ "$FUZZ" = true ]; then
		...
		if [ -s ".tmp/webs_all.txt" ]; then
			...
			if [ ! "$AXIOM" = true ]; then
				...
				axiom-exec 'wget -O /home/op/lists/fuzz_wordlist.txt https://raw.githubusercontent.com/omar Passos/OneListForAll/main/onelistforallmicro.txt' &>/dev/null
...
function sendToNotify {
	if [[ -z "$1" ]]; then
		...
		if [[ -n "$slack_channel" ]] && [[ -n "$slack_auth" ]]; then
			...
			curl -F file=@${1} -F "initial_comment=recondns zip file" -F channels=${slack_channel} -H "Authorization: Bearer ${slack_auth}" https://slack.com/api/files.upload &>/dev/null
		fi
	fi
}
...
function resolvers_update(){
	if [ "$generate_resolvers" = true ]; then
		if [ ! "$AXIOM" = true ]; then	
			...
			axiom-exec 'wget -O /home/op/lists/resolvers.txt https://raw.githubusercontent.com/BonJarber/fresh-resolvers/main/resolvers.txt' &>/dev/null
			axiom-exec 'wget -O /home/op/lists/resolvers_trusted.txt https://gist.githubusercontent.com/omar Passos/ae9ed7e5c786461868abd3f2344401b6/raw' &>/dev/null
			notification "Updated\n" good
		fi
...
function ipcidr_target(){
	...
	if [[ $1 =~ ^$IP_CIDR_REGEX ]]; then
		echo $1 | mapcidr -silent | anew -q target_recondns_ipcidr.txt
		if [ -s "./target_recondns_ipcidr.txt" ]; then 
			[ "$REVERSE_IP" = true ] && cat ./target_recondns_ipcidr.txt | dnsx -ptr -resp-only -silent -retry 3 | unfurl -u domains 2>/dev/null | sed 's/\.$//' | anew -q ./target_recondns_ipcidr.txt
			if [[ $(cat ./target_recondns_ipcidr.txt | wc -l) -eq 1 ]]; then
				domain=$(cat ./target_recondns_ipcidr.txt)
			elif [[ $(cat ./target_recondns_ipcidr.txt | wc -l) -gt 1 ]]; then
				unset domain
				list=${PWD}/target_recondns_ipcidr.txt
			fi
		fi
		if [ -n "$2" ]; then
			cat $list | anew -q $2
...
PROGARGS=$(getopt -o 'd:m:l:x:i:o:f:c:rspanwvh::' --long 'domain:,list:,recon,subdomains,passive,all,web,osint,deep,help,vps' -n 'recondns' -- "$@")
...
case $opt_mode in
        ...
        'w')
			if [ -n "$list" ]; then
				...
				printf "\n\n${bred} Web mode needs a website list file as target (./recondns.sh -l target.txt -w) ${reset}\n\n"
				exit
			fi
			webs_menu
			exit
